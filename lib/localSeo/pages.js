...
/** Footer: e-ticaret / mağaza odaklı sayfalar (şehir listesinden ayrı blok) */
export function getStoreFocusedPages() {
  return LOCAL_SEO_PAGES.filter(
    (p) => p.slug.includes('e-ticaret') || p.slug.includes('magaza')
  ).map((p) => ({
    slug: p.slug,
    label: getPageLinkLabel(p.slug),
    cityName: p.cityName,
