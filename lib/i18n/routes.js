import { isLandingSlug } from '@/lib/landingPages/routes'
...
export function toEnglishPath(pathname) {
  ...
  // Turkish-only landing pages have no translated counterpart. Keep the
  // existing language switch usable without sending it to a 404 or TR page.
  if (pathname.startsWith('/tr/') && isLandingSlug(pathname.slice(4).replace(/\/$/, ''))) return '/services'
