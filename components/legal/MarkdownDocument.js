...
function inline(text, key) {
  ...
  return tokens.map((token, index) => {
    ...
    if (link) return link[2].startsWith('http') || link[2].startsWith('mailto:')
      ...
      : <Link key={`${key}-${index}`} href={link[2]}>{link[1]}</Link>
...
export default function MarkdownDocument({ source }) {
  const blocks = source.trim().split(/\n\s*\n/)
  return <article className="legal-document">
    {blocks.map((block, index) => {
      if (block.startsWith('# ')) return <h1 key={index}>{inline(block.slice(2), index)}</h1>
      if (block.startsWith('## ')) return <h2 key={index}>{inline(block.slice(3), index)}</h2>
      if (block.startsWith('### ')) return <h3 key={index}>{inline(block.slice(4), index)}</h3>
      if (block.split('\n').every((line) => line.startsWith('- '))) return <ul key={index}>{block.split('\n').map((line, item) => <li key={item}>{inline(line.slice(2), `${index}-${item}`)}</li>)}</ul>
